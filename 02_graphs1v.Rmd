...
```{r bla, fig.cap='Densidad para la variable peso en la izquierda, densidad para el peso diferenciando por sexo a la derecha..', fig.asp=0.4, fig.width =12}
url <- 'https://raw.githubusercontent.com/fhernanb/datos/master/medidas_cuerpo'
datos <- read.table(file=url, header=T)

par(mfrow=c(1, 2))
plot(density(datos$peso), main='Densidad para el peso corporal',
     xlab='Peso corporal (kg)', ylab='Densidad', lwd=3)
...
plot(den.hom, xlim=c(20, 120), 
     ...
     xlab='Peso corporal (kg)', lwd=3, col='blue')
lines(den.muj, lwd=3, col='red')
