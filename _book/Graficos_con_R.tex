...
\item
  \texttt{type}: tipo de línea a usar y están disponibles ``blank'',
  ``solid'', ``dashed'', ``dotted'', ``dotdash'', ``longdash'' o
  ``twodash''.
\item
...
A continuación se muestra un ejemplo de cómo dibujar dos funciones
\(f_1\) y \(f_2\) usando la función \texttt{curve}.

La ecuación para \(f_1\) es:

\[f_1(x)=x^2-10\] La ecuación para \(f_2\) es: \[
f(n) = \begin{cases} x^2-10, & \mbox{si } x < 1 \\ 5 x, & \mbox{si } x \geq 1 \end{cases}
\] El código necesario para definir las dos funciones se muestra a
continuación. Se Para dibujar \(f_2\) se usó \texttt{add=TRUE} con el
objetivo de que quedaran ambas en una misma ventana gráfica. Note que se
usó \texttt{xlim=c(-5,\ 10)} para indicar que se deseaba una ventana
gráfica en ese intervalo para el eje horizontal.
...
\KeywordTok{curve}\NormalTok{(}\DataTypeTok{expr=}\NormalTok{f2, }\DataTypeTok{from=}\NormalTok{-}\DecValTok{2}\NormalTok{, }\DataTypeTok{to=}\DecValTok{4}\NormalTok{, }\DataTypeTok{type=}\StringTok{'l'}\NormalTok{,}
      \DataTypeTok{col=}\StringTok{'green4'}\NormalTok{, }\DataTypeTok{lwd=}\DecValTok{4}\NormalTok{, }\DataTypeTok{add=}\OtherTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\caption{\label{fig:curve1}Dibujo de dos funciones en una misma ventana.}
\end{figure}
