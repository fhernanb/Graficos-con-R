...
siguiente advertencia: ``\emph{Pie charts are a very bad way of
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{barplot}\NormalTok{(x, labels)}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{pie}\NormalTok{(}\DataTypeTok{x=}\NormalTok{tabla1, }\DataTypeTok{labels=}\NormalTok{nombres, }
    \DataTypeTok{main=}\StringTok{'Gráficos de pastel NO recomendados'}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\index{par}}{Función par }}\label{funcion-par-1}
