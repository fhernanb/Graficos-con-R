...
Esta función es muy útil para digujar polígonos. La estructura de la función se muestra a continuación.
...
```{r poly1, fig.cap='Ejemplo de formas obtenidas con `polygon`.', fig.asp=0.8, fig.width=6}
...
- `type`: tipo de línea a usar y están disponibles "blank", "solid", "dashed", "dotted", "dotdash", "longdash" o "twodash".
- `xlim`: intervalo para construir la ventana gráfica, no confundir con `from` y `to`.


A continuación se muestra un ejemplo de cómo dibujar dos funciones $f_1$ y $f_2$ usando la función `curve`.

La ecuación para $f_1$ es:

$$f_1(x)=x^2-10$$
La ecuación para $f_2$ es:
$$
f(n) = \begin{cases} x^2-10, & \mbox{si } x < 1 \\ 5 x, & \mbox{si } x \geq 1 \end{cases}
$$
El código necesario para definir las dos funciones se muestra a continuación. Se  Para dibujar $f_2$ se usó `add=TRUE` con el objetivo de que quedaran ambas en una misma ventana gráfica. Note que se usó `xlim=c(-5, 10)` para indicar que se deseaba una ventana gráfica en ese intervalo para el eje horizontal.

```{r curve1, fig.cap='Dibujo de dos funciones en una misma ventana.', fig.asp=0.8, fig.width=5}
...
curve(expr=f2, from=-2, to=4, type='l',
      col='green4', lwd=4, add=TRUE)
