...
El código necesario para obtener la figura se muestra a continuación. Note que a pesar de haber solicitado título principal y nombres de los ejes, éstos no aparecen en el resultado final. En la Figura \@ref(fig:ann) se muestra el gráfico solicitado.
...
```{block2, type='rmdnote'}
La opción `ann=FALSE` es muy útil para gráficos sin nombres en los ejes y sin título principal. Esta opción evita el uso de `xlab='', ylab='', main=''` dentro de las funciones gráfica.
```
...
El código necesario para obtener lo solicitado se muestra a continuación. Observando la Figura \@ref(fig:adj) se nota claramente que cuando `adj=0.5` la palabra queda centrada mientras que con otros valores cambia la justificación del texto.
